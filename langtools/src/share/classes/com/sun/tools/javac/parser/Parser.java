...
//todo: skip to anchor on error(?)
     ...
     *                     [IMPLEMENTS TypeAndContractList] ClassBody
    ...
    JCClassDecl classDeclaration(JCModifiers mods, String dc) {
        ...
        if (S.token() == IMPLEMENTS) {
            S.nextToken();
            implementing = typeAndContractList(); // CONTRACTS
        }
    ...
    /** EnumDeclaration = ENUM Ident [IMPLEMENTS TypeAndContractList] EnumBody
    ...
    /** TypeAndContractList = TypeOrContract {"," TypeOrContract}
     */
    List<JCExpression> typeAndContractList() { // CONTRACTS
        ListBuffer<JCExpression> ts = new ListBuffer<JCExpression>();
        ts.append(typeOrContract());
        while (S.token() == COMMA) {
            S.nextToken();
            ts.append(typeOrContract());
        }
    ...
    /** TypeOrContract = [STATIC] Type
     */
    JCExpression typeOrContract() { // CONTRACTS
    	int pos = S.pos();
    	if (S.token() == STATIC) {
    		S.nextToken();
    		return F.at(pos).Contract(type());
    	}
   		return type();
    }
